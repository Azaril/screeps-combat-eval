...
fn main() {
    ...
    let mut cache: Cache = std::fs::read_to_string(out_path)
        .ok()
        .and_then(|s| serde_json::from_str(&s).ok())
        .unwrap_or_default();
    let have: std::collections::HashSet<String> =
        cache.bases.iter().map(|b| b.room.clone()).collect();
    ...
    let todo: Vec<_> = fx
        ...
        .collect();
    println!(
        "capture_base: {} fixtures, {} already cached, planning {} (foreman is SLOW)…",
        fx.len(),
        have.len(),
        todo.len()
    );
    ...
    for (i, fixture) in todo.iter().enumerate() {
        ...
        match capture(fixture) {
            Ok(base) => {
                let (spawns, towers, ramparts, walls) =
                    base.structures
                        .iter()
                        .fold((0, 0, 0, 0), |(s, t, r, w), st| match st.kind.as_str() {
                            "spawn" => (s + 1, t, r, w),
                            "tower" => (s, t + 1, r, w),
                            "rampart" => (s, t, r + 1, w),
                            _ => (s, t, r, w + 1),
    ...
    println!(
        "capture_base: done — {} bases cached in {out_path}",
        cache.bases.len()
    );
}
